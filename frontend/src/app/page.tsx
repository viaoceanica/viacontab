...
export default function Home() {
  ...
  /* ── Overlay backdrop (shared by drawer + modals) ── */
  ...
  /* ── Drawer (slide-in from right) for invoice detail ── */
  const drawerStyle: React.CSSProperties = {
    ...
    transform: detailInvoice ? "translateX(0)" : "translateX(100%)",
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f172a, #1d4ed8)", padding: 24 }}>
      ...
      {/* ══════════════════════════════════════════════════════
          DRAWER — Invoice detail (slides in from right)
         ══════════════════════════════════════════════════════ */}
      {detailInvoice && <div style={overlayStyle} onClick={closeDetails} />}
        ...
        {detailInvoice && (
          <>
            ...
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 18 }}>Detalhes da fatura</h3>
              ...
              <button
                ...
                style={{ width: 36, height: 36, borderRadius: 10, border: "1px solid #cbd5f5", background: "#f8fafc", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}
              >
                ✕
              </button>
            </div>
      ...
      {/* ══════════════════════════════════════════════════════
          SETTINGS MODAL — Tenant profile
         ══════════════════════════════════════════════════════ */}
      {isSettingsOpen && (
        <div style={modalBackdropStyle} onClick={() => setIsSettingsOpen(false)}>
          <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <h3 style={{ margin: 0, fontSize: 20 }}>Configurações</h3>
              <button
            ...
            <h4 style={{ margin: "0 0 12px", color: "#334155" }}>Dados da empresa do tenant</h4>
            ...
            <div style={{ marginTop: 16, display: "flex", gap: 12 }}>
              ...
              >
                {isSavingTenantProfile ? "A guardar…" : "Guardar"}
              </button>
              <button
                onClick={() => setIsSettingsOpen(false)}
      ...
      {/* ══════════════════════════════════════════════════════
          CHAT — Collapsible floating panel (bottom-right)
         ══════════════════════════════════════════════════════ */}
      {isChatOpen ? (
        <div
          style={{
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <strong style={{ fontSize: 15 }}>Chat inteligente</strong>
            </div>
            <button
              onClick={() => setIsChatOpen(false)}
              style={{ width: 32, height: 32, borderRadius: 8, border: "1px solid #cbd5f5", background: "white", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}
            >
              ✕
            </button>
          </div>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 12 }}>
            {chatHistory.length === 0 ? (
              <div style={{ color: "#94a3b8", fontSize: 14 }}>Sem conversas ainda. Experimente: &quot;Quanto gastei com Via Oceânica este mês?&quot;</div>
            ) : (
          ...
          {/* Chat input */}
          <div style={{ padding: "12px 20px 16px", borderTop: "1px solid #e2e8f0", flexShrink: 0 }}>
            {chatError && <div style={{ marginBottom: 8, color: "#ef4444", fontSize: 13 }}>{chatError}</div>}
            <textarea
              placeholder="Ex.: Total de IVA para Via Oceânica"
              value={chatInput}
              onChange={(event) => setChatInput(event.target.value)}
              rows={2}
              style={{ width: "100%", borderRadius: 12, border: "1px solid #cbd5f5", padding: 10, fontSize: 14, resize: "none" }}
            />
            <button
              onClick={() => void handleSendChat()}
              disabled={isChatLoading || !chatInput.trim()}
              style={{ marginTop: 8, width: "100%", padding: "10px 14px", borderRadius: 12, border: "none", background: isChatLoading ? "#94a3b8" : "#22c55e", color: "white", fontWeight: 600 }}
            >
