...
function toPortugueseFailedImportReason(row: FailedImportRow) {
  const reason = String(row.reason || "");
  const lowered = reason.toLowerCase();
  const detected = String(row.detected_type || "").toLowerCase();

  if (detected === "processing_timeout" || lowered.includes("timeout") || lowered.includes("tempo limite")) {
    return "Tempo limite excedido durante extração (90s por ficheiro)";
  }
  if (detected === "payment_certificate" || lowered.includes("payment certificate")) {
    return "Pré-check: documento identificado como comprovativo de pagamento";
  }
  if (detected === "bank_statement" || lowered.includes("bank statement")) {
    return "Pré-check: documento identificado como extrato bancário";
  }
  if (lowered.includes("could not validate the document as an invoice")) {
    return "Não foi possível validar o documento como fatura ou documento fiscal aceite";
  }
  if (detected === "processing_error") {
    return "Falha técnica ao analisar o documento";
  }
  return reason;
}
...
export default function Home() {
  ...
  const [selectedFailedImportIds, setSelectedFailedImportIds] = useState<string[]>([]);
  ...
  const queueFailedImportDelete = useCallback(
    (row: FailedImportRow, skipConfirm = false) => {
      const confirmed = skipConfirm ? true : window.confirm(`Remover falha ${row.filename}?`);
      if (!confirmed) return;

      setFailedImports((prev) => prev.filter((item) => item.id !== row.id));
      setSelectedFailedImportIds((prev) => prev.filter((id) => id !== row.id));
      const timeoutId = window.setTimeout(() => {
  ...
  const handleBulkFailedImportDelete = useCallback(() => {
    if (selectedFailedImportIds.length === 0) return;
    const confirmed = window.confirm(`Remover ${selectedFailedImportIds.length} falha(s) selecionada(s)?`);
    if (!confirmed) return;

    const map = new Map(failedImports.map((row) => [row.id, row]));
    selectedFailedImportIds.forEach((failedId) => {
      const row = map.get(failedId);
      if (row) queueFailedImportDelete(row, true);
    });
    setSelectedFailedImportIds([]);
  }, [selectedFailedImportIds, failedImports, queueFailedImportDelete]);
  ...
  const sortedFailedImports = useMemo(() => {
    const rank = (row: FailedImportRow) => {
      const reason = `${row.reason || ""} ${row.detected_type || ""}`.toLowerCase();
      const lineCorrection =
        reason.includes("linha") ||
        reason.includes("line") ||
        reason.includes("correc") ||
        reason.includes("correç");
      const timeout =
        reason.includes("tempo limite") ||
        reason.includes("timeout") ||
        (row.detected_type || "").toLowerCase() === "processing_timeout";

      if (lineCorrection) return 0;
      if (timeout) return 2;
      return 1;
    };

    return [...failedImports].sort((a, b) => {
  ...
  const allFailedVisibleSelected =
    sortedFailedImports.length > 0 && sortedFailedImports.every((row) => selectedFailedImportIds.includes(row.id));
  ...
  return (
    <div className="app-shell min-h-screen bg-ocean-bg text-ocean-text">
          ...
          <>
            ...
            <section className="card queue-support-failed">
              ...
              <div className="actions-row" style={{ marginBottom: 8 }}>
                <button className="danger-btn" disabled={selectedFailedImportIds.length === 0} onClick={handleBulkFailedImportDelete}>
                  Remover selecionadas ({selectedFailedImportIds.length})
                </button>
                <button
                  className="ghost-btn"
                  disabled={selectedFailedImportIds.length === 0}
                  onClick={() => setSelectedFailedImportIds([])}
                >
                  Limpar seleção
                </button>
              </div>
              {sortedFailedImports.length === 0 ? (
                ...
                <div className="table-wrap">
                  <table>
                    <thead>
                      <tr>
                        <th>
                          <input
                            type="checkbox"
                            checked={allFailedVisibleSelected}
                            onChange={(event) => {
                              if (event.target.checked) {
                                setSelectedFailedImportIds(sortedFailedImports.map((row) => row.id));
                              } else {
                                setSelectedFailedImportIds([]);
                              }
                            }}
                    ...
                    <tbody>
                      {sortedFailedImports.map((row) => (
                        <tr key={row.id}>
                          <td>
                            <input
                              type="checkbox"
                              checked={selectedFailedImportIds.includes(row.id)}
                              onChange={(event) => {
                                setSelectedFailedImportIds((prev) => {
                                  if (event.target.checked) return [...prev, row.id];
                                  return prev.filter((id) => id !== row.id);
                                });
                              }}
                              aria-label={`Selecionar falha ${row.filename}`}
                            />
                          </td>
                          <td>{row.filename}</td>
                          <td>{toPortugueseFailedImportReason(row)}</td>
